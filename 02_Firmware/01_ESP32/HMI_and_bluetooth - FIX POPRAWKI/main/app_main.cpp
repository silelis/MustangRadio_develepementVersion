...
extern "C" void app_main(void)
{
	...
	// Taski I2C slave jawnie przypinamy do Core 0, aby ISR I2C slave pozostal na Core 0.
	// ISR I2C master zostal przeniesiony na Core 1 (patrz stepperMotorDataParser).
	// Rozdzielenie: Core 0 = I2C slave, Core 1 = I2C master + stepper.
	//configASSERT(xTaskCreate(i2cSlaveTransmit, "I2C slave tx", 128 * 8, NULL, tskIDLE_PRIORITY+1, &handlerTask_i2cSlaveTransmit));   // stary kod - nieprzypiete
	//configASSERT(xTaskCreate(i2cSlaveReceive, "I2C slave rx", 128 * 10, NULL, tskIDLE_PRIORITY+2, &handlerTask_i2cSlaveReceive));      // stary kod - nieprzypiete
	//configASSERT(xTaskCreate(i2cSlaveTransmit, "I2C slave tx", 128 * 32, NULL, tskIDLE_PRIORITY + 2, &handlerTask_i2cSlaveTransmit)); // stary kod - nieprzypiete
	configASSERT(xTaskCreatePinnedToCore(i2cSlaveReceive,  "I2C slave rx", 128 * 10, NULL, tskIDLE_PRIORITY + 2, &handlerTask_i2cSlaveReceive,  0));
	configASSERT(xTaskCreatePinnedToCore(i2cSlaveTransmit, "I2C slave tx", 128 * 32, NULL, tskIDLE_PRIORITY + 2, &handlerTask_i2cSlaveTransmit, 0));
	...
	// [FIX: Guru Meditation - xPortEnterCriticalTimeout deadlock]
	// KLUCZOWA ZMIANA: task musi byc pinnedToCore1, poniewaz inicjalizuje I2C master bus
	// wewnatrz siebie (i2c_new_master_bus() przypisuje ISR do rdzenia wywolujacego).
	// Wczesniej: xTaskCreate (Core 0) -> ISR mastera na Core 0 -> deadlock z i2cSlaveTransmit.
	// Teraz:     pinnedToCore1        -> ISR mastera na Core 1 -> brak konfliktu.
	//assert(xTaskCreate(stepperMotorDataParser, "Stepper morot", 128 * 20, NULL, tskIDLE_PRIORITY, &handlerTask_stepperMotorDataPasrser)); // stary kod - nieprzypiete, powodowalo crash
	...
	assert(xTaskCreatePinnedToCore(stepperMotorDataParser, "Stepper morot", 128 * 20, NULL, tskIDLE_PRIORITY, &handlerTask_stepperMotorDataPasrser, TASK_TO_CORE1));
