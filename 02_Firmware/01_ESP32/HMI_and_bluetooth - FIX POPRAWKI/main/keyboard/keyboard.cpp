...
DRAM_ATTR volatile static bool suspendLongPressFeeder = false;
...
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
		...
		if (suspendLongPressFeeder) {
			suspendLongPressFeeder = false;
			vTaskSuspend(NULL);
		}
	}
}
...
KEYBOARD::KEYBOARD(QueueHandle_t queueHandler_Keyboard, TaskHandle_t taskHandler_onPeriodLongButtonPressNotification)
{
	...
	// [BUG FIX] Błąd: nazwa parametru konstruktora była identyczna z nazwą zmiennej globalnej.
	// Skutek: przypisanie było samo do siebie (self-assignment) - globalna zostawała NULL.
	// Przy długim naciśnięciu przycisku ISR wywoływał xTaskResumeFromISR(NULL) -> assert.
	// Błąd był maskowany przez wcześniejszy Guru Meditation crash (WDT deadlock).
	// Naprawa: użycie operatora :: aby jawnie wskazać zmienną globalną.
