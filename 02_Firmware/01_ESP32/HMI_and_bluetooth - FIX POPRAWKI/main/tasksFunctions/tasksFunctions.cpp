...
*/
	...
	// Semafor musi byc stworzony TUTAJ (synchronicznie, przed startem taskow), a nie
	// w stepperMotorDataParser, bo i2cReceivedDataParser moglby probowac Take() na NULL.
	// Binary semaphore startuje w stanie "taken" (Count=0) - Give() wyda stepperMotorDataParser
	// po zakonczeniu inicjalizacji pMotor na Core 1.
	xMotorInitDone = xSemaphoreCreateBinary();
	configASSERT(xMotorInitDone);
	...
	// [FIX: Guru Meditation - xPortEnterCriticalTimeout deadlock]
	// STARY KOD - i2c_new_master_bus() wywolywane z Core 0 przypisywalo ISR do Core 0.
	// Powodowalo deadlock: ISR mastera (Core 0) vs. xKernelLock trzymany przez task
	// stepera czekajacy na Core 1. Skutek: Interrupt WDT -> crash przy wcisnieciu
	// klawisza Power ON/OFF (start kalibracji stepera).
	// Inicjalizacja przeniesiona do stepperMotorDataParser() w tasksFunctionsStepperMotor.cpp
	// ktory jest teraz pinnedToCore1 -> ISR mastera przypisany do Core 1.
	/*
...
static BaseType_t esp32PrepareKbrdDataAndSent_to_QueueSend(const i2cFrame_keyboardFrame * pvItemToQueue, size_t itemSize)
{
	...
	dataToTransmitQueue.pData = /*new char[sizeof(itemSize)];*/ new char[itemSize];		//alokuje bufor o rozmiarze struktury i2cFrame_keyboardFrame, do którego zostaną skopiowane dane z klawiatury, a który następnie zostanie wysłany do kolejki i2c slave transmit queue
...
{
	// [FIX: Guru Meditation - xPortEnterCriticalTimeout deadlock]
	// pMotor jest teraz inicjalizowany asynchronicznie na Core 1 (stepperMotorDataParser).
	// Czekamy na semafor xMotorInitDone zanim uzyjem pMotor (case I2C_COMMAND_GROUP_STEPPER).
	xSemaphoreTake(xMotorInitDone, portMAX_DELAY);
	xSemaphoreGive(xMotorInitDone); // oddajemy natychmiast - to jednorazowa bariera startowa
