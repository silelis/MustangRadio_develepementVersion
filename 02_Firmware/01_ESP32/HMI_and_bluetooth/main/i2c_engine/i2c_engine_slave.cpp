...
QueueHandle_t DRAM_ATTR i2cSlaveCallbackEventQueue =  xQueueCreate( 10, sizeof(i2c_slave_event_context_t) );
...
{
	i2c_slave_event_context_t evt;
	evt.event_type = I2C_SLAVE_EVT_TX;
    BaseType_t xTaskWoken = 0;
	xQueueSendFromISR(i2cSlaveCallbackEventQueue , &evt, &xTaskWoken);
    return xTaskWoken;
}
...
i2cEngin_slave::i2cEngin_slave(i2c_port_num_t i2c_port, gpio_num_t sda_io_num, gpio_num_t scl_io_num, uint32_t slave_addr, i2c_addr_bit_len_t slave_addr_bit_len, gpio_num_t intRequestPin)
{
	...
	i2c_config_slave.send_buf_depth = ESP32_SLAVE_RECEIVE_BUFFER_LEN;
	i2c_config_slave.receive_buf_depth = ESP32_SLAVE_RECEIVE_BUFFER_LEN;
	i2c_config_slave.scl_io_num = scl_io_num;
	i2c_config_slave.sda_io_num = sda_io_num;
	i2c_config_slave.slave_addr = slave_addr;
	i2c_config_slave.flags.allow_pd = 0;
    i2c_config_slave.flags.enable_internal_pullup = true;
    i2c_config_slave.intr_priority = 3;
...
//	rx_fifo_end_addrLast = I2C0.fifo_st.rx_fifo_end_addr;
	...
	ESP_ERROR_CHECK(i2c_slave_register_event_callbacks(handler_i2c_dev_slave, &cbs, NULL));
	printf("%s bus has been initialised on port %d with address %lx.\n", this->TAG, i2c_port, slave_addr);
...
//	ESP_ERROR_CHECK(i2c_slave_receive(handler_i2c_dev_slave, data_rd, ESP32_SLAVE_RECEIVE_BUFFER_LEN));
	...
	while (1)
	{
		...
		if (xQueueReceive(i2cSlaveCallbackEventQueue, &callbackEvtData, portMAX_DELAY) == pdTRUE){
