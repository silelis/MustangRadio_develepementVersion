...
private:
	HAL_StatusTypeDef masterReceiveFromESP32_DMA(uint8_t *pData, uint16_t Size);
	HAL_StatusTypeDef masterReceiveFromESP32_DMA_inSequence(uint8_t *pData,
...
#warning zrobic obsługę błędów komunikacji (jeśli są błędy komunikacji i jeśli nie ma sygnału keep alive
	...
	void giveESP32I2CInterfaceTime(void);
};
