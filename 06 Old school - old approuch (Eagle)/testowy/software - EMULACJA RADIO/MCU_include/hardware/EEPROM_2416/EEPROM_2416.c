...
void write_ext_eeprom(EEPROM_ADDRESS address, void *data, unsigned int16 _size_of_data) {
   ...
   i2c_MasterBuffer[0]   =  (EEPROM_I2C_WRITE_ADDRESS|(BYTE)(address>>7))&0xfe;
   i2c_MasterBuffer[1]   =  address;
   memcpy(&i2c_MasterBuffer+2, data, _size_of_data);
...
void read_ext_eeprom(EEPROM_ADDRESS address, void *data, EEPROM_ADDRESS _size_of_data) {
   ...
   i2c_MasterBuffer[0]   = ((EEPROM_I2C_WRITE_ADDRESS|(BYTE)(address>>7))&0xfe);
   i2c_MasterBuffer[1]   = (address);
   i2c_MasterBuffer[2]   = ((EEPROM_I2C_WRITE_ADDRESS|(BYTE)(address>>7))|1);
   I2CWriteLength = 3;
   I2CReadLength = _size_of_data;
   i2cEngine(READ__EEPROM);
   memcpy(data, &i2c_SlaveBuffer, _size_of_data);
}
